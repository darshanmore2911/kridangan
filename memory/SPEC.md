...
## Frontend structure
`pages/Home.tsx` is a thin composer over section components in `components/home/`: `Hero` (composes `HeroContent` + `HeroCommandPanel`), `EventIntro`, `GamesSection`, `PrizePool`, `Highlights`, `AboutSection`, `RegistrationCTA`. `RegistrationModal` renders one `RegistrationOption` per game (link when a form URL is set, disabled COMING SOON button otherwise). Sections that trigger registration take an `onRegister` prop; the modal state lives in `Home`. All components declare explicit `ReactElement` return types and typed props/data interfaces.
